...
def start_Controller_248_AP():
    ...
    account = os.getenv('Controller_248_account')
    password = os.getenv('Controller_248_password')
    ...
    Controller_url = os.getenv('Controller_248_url')
    ...
    headers = {'Content-Type': 'text/plain'}
    cookie = {"SESSION": cookieStr}
    payloadData = 'query=<aruba_queries><query><qname>backend-observer-radio-28</qname><type>list</type><list_query><device_type>radio</device_type><requested_columns>ap_name radio_band total_data_bytes rx_data_bytes channel_str radio_mode eirp_10x max_eirp noise_floor arm_ch_qual sta_count current_channel_utilization rx_time tx_time channel_interference channel_free channel_busy avg_data_rate tx_avg_data_rate rx_avg_data_rate ap_quality</requested_columns><sort_by_field>ap_name</sort_by_field><sort_order>asc</sort_order><pagination><start_row>0</start_row><num_rows>400</num_rows></pagination></list_query></query></aruba_queries>&UIDARUBA='+cookieStr
    ...
    for i in range(len(df) - 1):
        if df['ap_name'][i] == df['ap_name'][i+1]:
            df['sta_count_all'][i] = df['sta_count'][i] + df['sta_count'][i+1]
    ...
    # seperate ap_name with '_': ex. IY_1F_AP03  => ap_group_building = IY, ap_group_floor = 1F, ap_group_name = AP03
